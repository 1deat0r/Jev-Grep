...
- Exact runtime/compiler/schema dependency pins, npm lockfile, strict TypeScript build, Node test runner, generated declarations, and a Linux CI workflow with pinned action commits. Hosted CI has not run yet.
...
Finish M1 request-context, administration/status, identity, and error-mapping contracts. Prototype a persistent matcher and production confined-read helper, retaining the exact bytes matched and measured resource bounds. Then connect a single safe exact-search path before adding the three transport adapters.
