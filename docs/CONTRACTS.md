...
Status: implemented structural search/read/administration/capability schemas and core search/read semantic validators. An in-process exact/read service now executes these envelopes. M1 administration/status and transport compatibility work remains in progress.
...
JSON Schema alone cannot express all of these guarantees. Each generated file explicitly identifies itself as structural validation only. Neither a structurally valid path nor matching scope IDs prove filesystem authorization; the workspace registry and native helper check actual policy and descriptor containment. Neither a matched digest field nor successful schema validation proves a live read happened; the verified-read implementation must establish that observation.
...
## Scoped service additions

`RequestContextSchema` defines per-call deadline and serialized-response budgets, including defaults and hard caps; optional deployment caps can only lower effective per-call limits. `EffectiveScope` adds optional authoritative include groups, effective policy hash and observed ignore snapshot hash; exclude arrays allow the union of two 64-pattern inputs. Registry-created policies and evidence identities use the canonical encodings in ADR 0003. The service validates final public results after applying envelope overhead. Known counts remain distinguished from unavailable aggregate counts.

## Remaining M1 work

Complete administrative response and index-status schemas; implement index configuration identity; freeze stable transport error mappings and additive-response compatibility behavior. Add conformance cases for those interfaces as they are finalized. These are explicitly pending rather than marked passed by the current contract tests.
